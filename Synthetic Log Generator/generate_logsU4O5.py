...
def get_decision(user, obj, action):
    ...
    if u_dept == o_type and action in ["read", "write"]:
        # High sensitivity write requires Manager + confidential clearance
        if o_sens == "High" and action == "write":
            if u_desig != "Manager" or u_clear != "confidential":
                return "Deny"
        # Medium sensitivity requires at least standard clearance
        if o_sens == "Medium" and u_clear == "basic":
            return "Deny"
        return "Allow"
    ...
    # Rule 3: Regional Low-sensitivity collaboration
    if action == "read" and o_sens == "Low" and u_reg == o_reg:
        return "Allow"
    
    # Rule 4: Cross-region Low read (global collaboration)
    if action == "read" and o_sens == "Low":
        return "Allow"
    
    # Rule 5: Manager override with clearance check
    if u_desig == "Manager" and action == "read" and o_sens == "Medium":
        if u_clear in ["standard", "confidential"]:
            return "Allow"
    
    # Rule 6: Confidential clearance can read High across departments
    if u_clear == "confidential" and action == "read" and o_sens == "High":
        return "Allow"
    
    # Rule 7: Finance can read Operational with clearance
    ...
    # Rule 8: Sales managers can read Financial with clearance
    if u_dept == "Sales" and u_desig == "Manager" and o_type == "Financial":
        if action == "read" and o_sens in ["Low", "Medium"] and u_clear in ["standard", "confidential"]:
            return "Allow"
    
    # Rule 9: HR compliance - can read Medium in same region
    if u_dept == "HR" and action == "read" and o_sens == "Medium":
        dept_region = DEPT_REGIONS.get(u_dept, "AMER")
        if dept_region == o_reg:
            return "Allow"
    
    # Rule 10: Owner department write access for Medium
    if u_dept == o_owner and action == "write" and o_sens == "Medium":
        if u_clear in ["standard", "confidential"]:
            return "Allow"
    
    # Rule 11: Same-region High read for Managers with confidential clearance
    if u_desig == "Manager" and u_clear == "confidential" and action == "read" and o_sens == "High":
        if u_reg == o_reg:
            return "Allow"
    
    # Rule 12: Cross-department Medium read within same region and clearance
    if action == "read" and o_sens == "Medium" and u_reg == o_reg:
        if u_clear in ["standard", "confidential"]:
            return "Allow"
...
def generate_users():
    ...
    for dept, roles in DEPARTMENTS.items():
        dept_region = DEPT_REGIONS.get(dept, "AMER")
        for i in range(len(roles) * 6):
            ...
            # 70% users in dept's primary region, 30% distributed
...
def generate_objects():
    ...
    for o_type, sens_list in OBJECTS_STRUCTURE.items():
        for sens in sens_list:
            for i in range(6):
                # 70% owner matches type, 30% distributed
                if random.random() < 0.7:
                    owner = o_type
                else:
                    owner = random.choice(list(DEPARTMENTS.keys()))
...
def format_for_file(entity, is_user=True):
    if is_user:
        return f"<{entity['user_name']} department:{entity['department']} designation:{entity['designation']} clearance:{entity['clearance']} region:{entity['region']}>"
    else:
...
def main():
    ...
    print(f"U4/O5 -> Final Log Ratio: {allow_count} Allow ({allow_count/total:.1%}) | {deny_count} Deny ({deny_count/total:.1%})")
