...
# --- Decision policy (balanced wrt user=3, object=2) ---
def get_decision(user, obj, action):
    u_dept, u_desig, u_reg = user["department"], user["designation"], user["region"]
    o_type, o_sens = obj["type"], obj["sensitivity"]

    # Region guard only when crossing departments on High reads
    if action == "read" and o_sens == "High" and u_dept != o_type:
        # Only IT can bypass regional restriction
        if u_dept != "IT":
            # Block if user is not in same broad region-family as the object's business domain
            domain_region = {
                "Financial": "AMER",
                "HR": "EMEA",
                "System": "APAC",
...
def format_for_file(entity, is_user=True):
    if is_user:
        return f"<{entity['user_name']} department:{entity['department']} designation:{entity['designation']} region:{entity['region']}>"
    else:
...
def main():
    ...
    print(f"U3/O2 -> Final Log Ratio: {allow_count} Allow ({allow_count/total:.1%}) | {deny_count} Deny ({deny_count/total:.1%})")
